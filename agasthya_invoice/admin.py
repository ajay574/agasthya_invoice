...
root.title("Agasthya Electronics (ADMIN)")
...
with sqlite3.connect("./Database/agasthya.db") as db:
    ...
    cur.execute("""CREATE TABLE if not exists raw_inventory(product_id int, product_name varchar(300), product_cat varchar(300), product_subcat varchar(250), stock int, mrp int, cost_price int, vendor_phn varchar(300))""");
    cur.execute(
        """CREATE TABLE if not exists employee(emp_id int, name varchar(100), contact_num bigint, address varchar(100), aadhar_num int, password varchar(20), designation varchar(200))""");
...
class login_page:
    def __init__(self, top=None):
        ...
        top.resizable(0, 0)
        top.title("Agasthya Electronics(ADMIN)")
    ...
    def login(self, Event=None):
        ...
        # with sqlite3.connect("./Database/agasthya.db") as db:
        #     cur = db.cursor()
        # find_user = "SELECT * FROM employee WHERE emp_id = ajay and password = 1234"
        # cur.execute(find_user, ["ajay", 1234])
        # results="Admin"
...
class Admin_Page:
    def __init__(self, top=None):
        ...
        self.button2.configure(text="""Products""")
...
def print_command(txt):
    ...
    print_btn=Button(bill,text="print",command=lambda:print_command(open_bill(1.0,END)))
    print_btn.place(relx=0.046, rely=0.055, width=136, height=130)
