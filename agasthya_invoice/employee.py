...
root.title("Agasthya Electronics")
...
with sqlite3.connect("./Database/agasthya.db") as db:
    cur = db.cursor()
...
def login(Event=None):
    ...
    with sqlite3.connect("./Database/agasthya.db") as db:
        cur = db.cursor()
        cur.execute(
            """CREATE TABLE if not exists raw_inventory(product_id int, product_name varchar(300), product_cat varchar(300), product_subcat varchar(250), stock int, mrp int, cost_price int, vendor_phn varchar(300))""");
        db.commit()
        cur.execute(
            """CREATE TABLE if not exists bill(bill_no varchar(300), date datetime, customer_name varchar(100), customer_no int, bill_details varchar(300))""");
        db.commit()
...
class login_page:
    def __init__(self, top=None):
        ...
        top.resizable(0, 0)
        top.title("Agasthya electronics")
...
class Item:
    def __init__(self, name, price, qty,Uprice):
...
class bill_window:
    ...
    def get_category(self, Event):
        ...
        self.combo3.configure(state="enbled")
            ...
            if(product_name!=""):
                ...
                if product_qty.isdigit()==True:
                    if (stock-int(product_qty))>=0:
                        ...
                        item = Item(product_name, mrp, int(product_qty),mrp)
                        self.cart.add_item(item)
                        self.Scrolledtext1.configure(state="normal")
                        bill_text = "{}\t\t\t\t\t{}\t\t\t\t {}\t\t\t\t{}\n ".format(product_name,product_qty,mrp,sp)
        ...
        if self.state == 1:
            ...
            if(cust_name.get()==""):
                ...
                if strr.find('Total')==-1:
                    ...
                    self.entry1.configure(state="normal", normalbackground="#ffffff", normalforeground="#000000")
                    self.entry2.configure(state="normal", normalbackground="#ffffff", normalforeground="#000000")
                    self.state = 0
        else:
            return
    ...
    clients = pd.read_sql('SELECT * FROM bill', db)
    clients.to_pdf('Clients100914.pdf', index=False)
